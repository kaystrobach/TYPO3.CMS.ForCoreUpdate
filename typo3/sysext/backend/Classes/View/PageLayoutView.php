...
class tx_cms_layout extends recordList {
	...
	public function getTable_pages($id) {
		...
		$delClause = (t3lib_BEfunc::deleteClause('pages') . ' AND ') . $GLOBALS['BE_USER']->getPagePermsClause(1);
		// Select current page:
		if (!$id) {
		...
		if (is_array($row)) {
			...
			foreach ($this->fieldArray as $field) {
				if ((($editIdList && isset($GLOBALS['TCA']['pages']['columns'][$field])) && $field != 'uid') && !$this->pages_noEditColumns) {
					...
					$eI = ((((('<a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($params, $this->backPath, ''))) . '" title="') . htmlspecialchars($iTitle)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-open')) . '</a>';
				} else {
					$eI = '';
				}
				switch ($field) {
				...
				default:
					if (substr($field, 0, 6) == 'table_') {
						$f2 = substr($field, 6);
						if ($GLOBALS['TCA'][$f2]) {
							$theData[$field] = '&nbsp;' . t3lib_iconWorks::getSpriteIconForRecord($f2, array(), array('title' => $GLOBALS['LANG']->sL($GLOBALS['TCA'][$f2]['ctrl']['title'], 1)));
						}
					} else {
				...
				foreach ($cList as $key) {
					...
					$content[$key] .= ((((('
						<div class="t3-page-ce-wrapper-new-ce">
							<a href="#" onclick="' . htmlspecialchars($this->newContentElementOnClick($id, $key, $lP))) . '" title="') . $GLOBALS['LANG']->getLL('newRecordHere', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-new')) . '</a>
						</div>
					';
					...
					foreach ($rowArr as $rKey => $row) {
						...
						if (is_array($row) && (int) $row['t3ver_state'] != 2) {
							...
							if (!$disableMoveAndNewButtons) {
								...
								$singleElementHTML .= ((((('<div style="margin-bottom:7px;margin-left:5px;"><a href="#" onclick="' . htmlspecialchars($onClick)) . '" title="') . $GLOBALS['LANG']->getLL('newRecordHere', 1)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-new')) . '</a></div>';
							}
	...
	public function makeOrdinaryList($table, $id, $fList, $icon = 0, $addWhere = '') {
		...
		// Column's titles
		if ($this->doEdit) {
			$theData['__cmds__'] = ((((('<a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick((((('&edit[' . $table) . '][') . $this->id) . ']=new'), $this->backPath))) . '" title="') . $GLOBALS['LANG']->getLL('new', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-new')) . '</a>';
		}
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
			...
			if (is_array($row)) {
				...
				if ($flag) {
					...
					// Attach edit icon
					if ($this->doEdit) {
						$Nrow['__editIconLink__'] = ((((('<a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($params, $this->backPath))) . '" title="') . $GLOBALS['LANG']->getLL('edit', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-open')) . '</a>';
					} else {
				...
				if ($GLOBALS['TCA'][$table]['columns'][$fieldName]) {
					...
					foreach ($theFields as $fName2) {
						if ($GLOBALS['TCA'][$table]['columns'][$fName2]) {
							$out[$fieldName] .= (((('<strong>' . $GLOBALS['LANG']->sL($GLOBALS['TCA'][$table]['columns'][$fName2]['label'], 1)) . '</strong>') . '&nbsp;&nbsp;') . htmlspecialchars(t3lib_div::fixed_lgd_cs(t3lib_BEfunc::getProcessedValue($table, $fName2, $row[$fName2], 0, 0, 0, $row['uid']), 25))) . '<br />';
						}
					}
				}
		...
		foreach ($fieldArr as $field) {
			...
			case 'uid':
				if ($GLOBALS['BE_USER']->doesUserHaveAccess($row, 2)) {
					...
					$eI = ((((('<a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($params, $this->backPath, ''))) . '" title="') . $GLOBALS['LANG']->getLL('editThisPage', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-open')) . '</a>';
				} else {
					$eI = '';
				}
	...
	public function tt_content_drawColHeader($colName, $editParams, $newParams) {
		...
		if ($this->tt_contentConfig['showCommands']) {
			...
			if ($editParams) {
				$icons .= ((((('<a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($editParams, $this->backPath))) . '" title="') . $GLOBALS['LANG']->getLL('editColumn', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-open')) . '</a>';
			}
		}
		if (strlen($icons)) {
	...
	public function tt_content_drawHeader($row, $space = 0, $disableMoveAndNewButtons = FALSE, $langMode = FALSE) {
		...
		if ($this->tt_contentConfig['showInfo']) {
			...
			if ($this->tt_contentConfig['showCommands'] && $this->doEdit) {
				...
				$out .= ((((('<a href="#" onclick="' . htmlspecialchars(t3lib_BEfunc::editOnClick($params, $this->backPath, ((t3lib_div::getIndpEnv('REQUEST_URI') . '#element-tt_content-') . $row['uid'])))) . '" title="') . htmlspecialchars(($this->nextThree > 1 ? sprintf($GLOBALS['LANG']->getLL('nextThree'), $this->nextThree) : $GLOBALS['LANG']->getLL('edit')))) . '">') . t3lib_iconWorks::getSpriteIcon('actions-document-open')) . '</a>';
				// Hide element:
				...
				if (!$disableMoveAndNewButtons) {
					...
					if ($this->tt_contentData['prev'][$row['uid']]) {
						...
						$moveButtonContent .= ((((('<a href="' . htmlspecialchars($GLOBALS['SOBE']->doc->issueCommand($params))) . '" title="') . $GLOBALS['LANG']->getLL('moveUp', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-move-up')) . '</a>';
					...
					if ($this->tt_contentData['next'][$row['uid']]) {
						...
						$moveButtonContent .= ((((('<a href="' . htmlspecialchars($GLOBALS['SOBE']->doc->issueCommand($params))) . '" title="') . $GLOBALS['LANG']->getLL('moveDown', TRUE)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-move-down')) . '</a>';
						$displayMoveButtons = TRUE;
					} else {
						$moveButtonContent .= t3lib_iconWorks::getSpriteIcon('empty-empty');
					}
		...
		if ($lockInfo = t3lib_BEfunc::isRecordLocked('tt_content', $row['uid'])) {
			$additionalIcons[] = ((((('<a href="#" onclick="' . htmlspecialchars((('alert(' . $GLOBALS['LANG']->JScharCode($lockInfo['msg'])) . ');return false;'))) . '" title="') . htmlspecialchars($lockInfo['msg'])) . '">') . t3lib_iconWorks::getSpriteIcon('status-warning-in-use')) . '</a>';
		}
				...
				if (!empty($row['records'])) {
					...
					foreach ($recordList as $recordIdentifier) {
						...
						if (is_array($shortcutRecord)) {
							...
							$shortcutContent[] = (((('<a href="#" onclick="' . htmlspecialchars($onClick)) . '">') . $icon) . '</a>') . htmlspecialchars(t3lib_BEfunc::getRecordTitle($tableName, $shortcutRecord));
						}
					}
	...
	public function numberOfRecords($table, $pid) {
		if ($GLOBALS['TCA'][$table]) {
			$count = $GLOBALS['TYPO3_DB']->exec_SELECTcountRows('uid', $table, (('pid=' . intval($pid)) . t3lib_BEfunc::deleteClause($table)) . t3lib_BEfunc::versioningPlaceholderClause($table));
		}
		return intval($count);
	}
		...
		foreach ($theTables as $tName) {
			...
			if ($GLOBALS['BE_USER']->check('tables_select', $tName) && ((t3lib_extMgm::isLoaded($tName) || t3lib_div::inList('fe_users,tt_content', $tName)) || isset($this->externalTables[$tName]))) {
				...
				if ($c || t3lib_div::inList('tt_content', $tName)) {
					// Add row to menu:
					$out .= ((('
					<td><a href="#' . $tName) . '"></a>') . t3lib_iconWorks::getSpriteIconForRecord($tName, array(), array('title' => $GLOBALS['LANG']->sL($GLOBALS['TCA'][$tName]['ctrl']['title'], 1)))) . '</td>';
					// ... and to the internal array, activeTables we also add table icon and title (for use elsewhere)
					$this->activeTables[$tName] = (t3lib_iconWorks::getSpriteIconForRecord($tName, array(), array('title' => (((($GLOBALS['LANG']->sL($GLOBALS['TCA'][$tName]['ctrl']['title'], 1) . ': ') . $c) . ' ') . $GLOBALS['LANG']->getLL('records', 1)))) . '&nbsp;') . $GLOBALS['LANG']->sL($GLOBALS['TCA'][$tName]['ctrl']['title'], 1);
				}
			}
		}
