...
class tx_sysaction_task implements tx_taskcenter_Task {
	...
	public function getTask() {
		...
		if ($show == 0) {
			...
			if (count($record) == 0) {
				$flashMessage = t3lib_div::makeInstance('t3lib_FlashMessage', $GLOBALS['LANG']->getLL('action_error-not-found', TRUE), $GLOBALS['LANG']->getLL('action_error'), t3lib_FlashMessage::ERROR);
				...
				case 5:
					...
					break;
				default:
					$flashMessage = t3lib_div::makeInstance('t3lib_FlashMessage', $GLOBALS['LANG']->getLL('action_noType', TRUE), $GLOBALS['LANG']->getLL('action_error'), t3lib_FlashMessage::ERROR);
	...
	protected function getActions() {
		...
		while ($actionRow = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
			...
			if ($GLOBALS['BE_USER']->isAdmin()) {
				...
				$editActionLink = (((((((('<a class="edit" href="' . $link) . '">') . '<img class="icon"') . t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'], 'gfx/edit2.gif')) . ' title="') . $GLOBALS['LANG']->getLL('edit-sys_action')) . '" alt="" />') . $GLOBALS['LANG']->getLL('edit-sys_action')) . '</a>';
			}
			$actionList[] = array(
				'uid' => $actionRow['uid'],
	...
	protected function renderActionList() {
		...
		if (count($actionList) > 0) {
			...
			$flashMessage = t3lib_div::makeInstance('t3lib_FlashMessage', $GLOBALS['LANG']->getLL('action_not-found-description', TRUE), $GLOBALS['LANG']->getLL('action_not-found'), t3lib_FlashMessage::INFO);
			$content .= $flashMessage->render();
		}
		...
		if ($GLOBALS['BE_USER']->isAdmin()) {
			...
			$content .= (((((((((('<br />
						<a href="' . $link) . '" title="') . $GLOBALS['LANG']->getLL('new-sys_action')) . '">') . '<img class="icon"') . t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'], 'gfx/new_record.gif')) . ' title="') . $GLOBALS['LANG']->getLL('new-sys_action')) . '" alt="" /> ') . $GLOBALS['LANG']->getLL('new-sys_action')) . '</a>';
		}
		return $content;
	}
	...
	protected function isCreatedByUser($id, $action) {
		$record = t3lib_BEfunc::getRecord('be_users', $id, '*', ((' AND cruser_id=' . $GLOBALS['BE_USER']->user['uid']) . ' AND createdByAction=') . $action['uid']);
		if (is_array($record)) {
			return $record;
		} else {
			return FALSE;
	...
	protected function getCreatedUsers($action, $selectedUser) {
		...
		$userList = array();
		// List of users
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', 'be_users', ((('cruser_id=' . $GLOBALS['BE_USER']->user['uid']) . ' AND createdByAction=') . intval($action['uid'])) . t3lib_BEfunc::deleteClause('be_users'), '', 'username');
	...
	protected function action_getUserMainDir() {
		...
		if (((($path && @is_dir($path)) && $GLOBALS['TYPO3_CONF_VARS']['BE']['lockRootPath']) && t3lib_div::isFirstPartOfStr($path, $GLOBALS['TYPO3_CONF_VARS']['BE']['lockRootPath'])) && substr($path, -1) == '/') {
			return $path;
		}
	}
		...
		if (t3lib_extMgm::isLoaded('lowlevel')) {
			...
			if (!is_array($sql_query) || is_array($sql_query) && strtoupper(substr(trim($sql_query['qSelect']), 0, 6)) === 'SELECT') {
				...
				if ($sqlQuery) {
					...
					$queryIsEmpty = TRUE;
					$flashMessage = t3lib_div::makeInstance('t3lib_FlashMessage', $GLOBALS['LANG']->getLL('action_emptyQuery', TRUE), $GLOBALS['LANG']->getLL('action_error'), t3lib_FlashMessage::ERROR);
					$content .= '<br />' . $flashMessage->render();
				}
				...
				if ($GLOBALS['BE_USER']->isAdmin()) {
					...
					$actionContent .= (((((((((((((('<br /><a title="' . $GLOBALS['LANG']->getLL('action_editQuery')) . '" href="') . $GLOBALS['BACK_PATH']) . t3lib_extMgm::extRelPath('lowlevel')) . 'dbint/index.php?id=') . '&SET[function]=search') . '&SET[search]=query') . '&storeControl[STORE]=-') . $record['uid']) . '&storeControl[LOAD]=1') . '">
						<img class="icon"') . t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'], 'gfx/edit2.gif')) . ' alt="" />') . $GLOBALS['LANG']->getLL(($queryIsEmpty ? 'action_createQuery' : 'action_editQuery'))) . '</a><br /><br />';
				}
			...
			$flashMessage = t3lib_div::makeInstance('t3lib_FlashMessage', $GLOBALS['LANG']->getLL('action_lowlevelMissing', TRUE), $GLOBALS['LANG']->getLL('action_error'), t3lib_FlashMessage::ERROR);
			$content .= '<br />' . $flashMessage->render();
		}
		return $content;
	}
	...
	protected function viewRecordList($record) {
		...
		if ($access) {
			...
			$flashMessage = t3lib_div::makeInstance('t3lib_FlashMessage', $GLOBALS['LANG']->getLL('action_error-access', TRUE), $GLOBALS['LANG']->getLL('action_error'), t3lib_FlashMessage::ERROR);
			$content .= $flashMessage->render();
		}
		return $content;
	}
