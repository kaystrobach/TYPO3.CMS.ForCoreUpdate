...
class DataHandler {
	...
	public function deleteSysFile($uid) {
		...
		if (!empty($fileObject) && $fileObject instanceof \TYPO3\CMS\Core\Resource\File) {
			try {
				...
				$this->BE_USER->writelog(4, 1, 112, 'You are not allowed to access the file', array($fileObject->getIdentifier()));
			} catch (\TYPO3\CMS\Core\Resource\Exception\NotInMountPointException $e) {
				$this->BE_USER->writelog(4, 1, 111, 'Target was not within your mountpoints! T="%s"', array($fileObject->getIdentifier()));
			} catch (\RuntimeException $e) {
				$this->BE_USER->writelog(4, 1, 110, 'Could not delete file "%s". Write-permission problem?', array($fileObject->getIdentifier()));
			}
			// Log success
			$this->BE_USER->writelog(4, 0, 1, 'File "%s" deleted', array($fileObject->getIdentifier()));
		} else {
