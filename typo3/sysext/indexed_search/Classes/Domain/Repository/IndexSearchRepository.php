...
*  Copyright notice
*
*  (c) 2011 Kasper Skårhøj (kasperYYYY@typo3.com), Benjamin Mack (benni@typo3.org)
*
*  All rights reserved
*
...
class Tx_IndexedSearch_Domain_Repository_IndexSearchRepository {
	...
	public function doSearch($searchWords, $freeIndexUid=-1) {
		...
		$res = $this->getSqlResult($searchWords, $freeIndexUid);
		$GLOBALS['TT']->pull();
	 ...
	 * @return	pointer
	 */
	protected function getSqlResult($searchWords, $freeIndexUid = -1) {
	 ...
	 */
	protected function mediaTypeWhere() {
		$whereClause = '';
	 ...
	 */
	protected function languageWhere() {
	 ...
	 */
	protected function freeIndexUidWhere($freeIndexUid) {
		...
		if (substr($this->sortOrder, 0, 5) == 'rank_') {
			...
			*/
			...
			switch ($this->sortOrder) {
				...
				// So, words are imploded into an OR statement (no "sentence search" 
				// should be done here - may deselect results)
