...
class tx_indexedsearch_modfunc1 extends t3lib_extobjbase {
	...
	public function drawTableOfIndexedPages() {
		...
		foreach ($tree->tree as $data) {
			$code .= $this->indexed_info($data['row'], $data['HTML'] . $this->showPageDetails(t3lib_BEfunc::getRecordTitlePrep($data['row']['title']), $data['row']['uid']));
		}
		if ($code) {
			$code = (('<br /><br />
	...
	public function printPhashRow($row, $grouping = 0, $extraGrListRows) {
		...
		case 2:
			...
			$lines[] = (((('<td style="white-space: normal;">' . htmlspecialchars(t3lib_div::fixed_lgd_cs($this->utf8_to_currentCharset($ftrow['fulltextdata']), 3000))) . '<hr/><em>Size: ') . strlen($ftrow['fulltextdata'])) . '</em>') . '</td>';
			// Query:
	...
	public function showDetailsForPhash($phash) {
		...
		if (is_array($phashRecord)) {
			$content .= '<h4>phash row content:</h4>' . $this->utf8_to_currentCharset(t3lib_utility_Debug::viewArray($phashRecord));
			...
			if (is_array($ftrows)) {
				...
				$content .= '<h3>Debug information:</h3>' . $this->utf8_to_currentCharset(t3lib_utility_Debug::viewArray($debugInfo));
	...
	public function printRemoveIndexed($phash, $alt) {
		return ((((('<a href="' . htmlspecialchars(t3lib_div::linkThisScript(array('deletePhash' => $phash)))) . '" title="') . htmlspecialchars($alt)) . '">') . t3lib_iconWorks::getSpriteIcon('actions-edit-delete')) . '</a>';
	}
	...
	public function printReindex($resultRow, $alt) {
		if ($resultRow['item_type'] && $resultRow['item_type'] !== '0') {
			return ((((((('<a href="' . htmlspecialchars(t3lib_div::linkThisScript(array('reindex' => $resultRow['phash'], 'reindex_id' => $resultRow['page_id'])))) . '">') . '<img ') . t3lib_iconWorks::skinImg($GLOBALS['BACK_PATH'], 'gfx/refresh_n.gif', 'width="14" height="14"')) . ' hspace="1" vspace="2" border="0" title="') . htmlspecialchars($alt)) . '" alt="" />') . '</a>';
		}
	}
