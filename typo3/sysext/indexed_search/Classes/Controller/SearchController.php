...
class Tx_IndexedSearch_Controller_SearchController extends Tx_Extbase_MVC_Controller_ActionController {
	...
	protected function getDisplayResults($searchWords, $resultData, $freeIndexUid = -1) {
		...
		if ($resultData) {
			...
			$result['rows'] = $this->compileResultRows($resultData['resultRows'], $freeIndexUid);
				...
				// Browsing box
			if ($resultData['count']) {
				$this->internal['res_count'] = $resultData['count'];
				$this->internal['results_at_a_time'] = $this->searchData['results'];
				$this->internal['maxPages'] = t3lib_div::intInRange($this->conf['search.']['page_links'], 1, 100, 10);

				if ($resData['count'] && $this->searchData['group'] == 'sections' && $freeIndexUid <= 0) {
				 	$addString = ' ' . sprintf(Tx_Extbase_Utility_Localization::translate('result.' . (count($this->resultSections) > 1 ? 'inNsections' : 'inNsection'), 'indexed_search'), count($this->resultSections));
				}
				// Browsing nav, bottom
				$result['browsebox1'] = $this->pi_list_browseresults(1, $addString, $this->printResultSectionLinks(), $freeIndexUid);
				$result['browsebox2'] = $this->pi_list_browseresults(0, '', '', $freeIndexUid);
			}
		}

			// Print a message telling which words we searched for, and in which sections etc.
		$what = $this->tellUsWhatIsSeachedFor($searchWords);
	
		if (substr($this->searchData['sections'], 0, 2) == 'rl') {
			$what .= Tx_Extbase_Utility_Localization::translate('result.inSection', 'indexed_search')
			 . ' "' . substr($this->getPathFromPageId(substr($this->searchData['sections'], 4)), 1) . '"';
		}
		$result['whatissearchedfor'] = $what;
		return $result;
	}
	...
	function makeItemTypeIcon($it, $alt='', $specRowConf) {
		...
		if (!isset($this->iconFileNameCache[$it]))	{
			...
			if (is_array($this->conf['iconRendering.']))	{
				...
				} elseif ($this->externalParsers[$it]) {
					$icon = $this->externalParsers[$it]->getIcon($it);
				}
	 ...
	 * @todo overwork this
	 */
	protected function makeDescription($row,$noMarkup=0,$lgd=180) {
		if ($row['show_resume']) {
			if (!$noMarkup)	{
				$markedSW = '';
...
// debug($parts,$regExString);
	...
	protected function writeSearchStat($searchParams, $searchWords, $count, $pt) {
		$insertFields = array(
			...
			'feuser_id' => intval($GLOBALS['TSFE']->fe_user->user['uid']),			// fe_user id, integer
			'cookie' => $GLOBALS['TSFE']->fe_user->id,						// cookie as set or retrieve. If people has cookies disabled this will vary all the time...
	...
	protected function getSearchWords($defaultOperator) {
		...
		if ($hookObj = $this->hookRequest('getSearchWords')) {
			return $hookObj->getSearchWords_splitSWords($searchWords, $defaultOperator);
		} else {
	 ...
	 */
	protected function linkPage($id, $str, $row=array(), $markUpSwParams = array()) {
		...
		if (count($this->domainRecords[$id])) {
			...
			$uri = $scheme . $firstDomain . '/index.php?id=' . $id . $addParams;

			if ($target = $this->settings['detectDomainRecords.']['target']) {
				$target = ' target="' . $target . '"';
			}
			return '<a href="' . htmlspecialchars($uri) . '"' . $target . '>' . htmlspecialchars($str) . '</a>';
		} else {
			// FIXME: use uribuilder
			return 'NOLINK';
			// return $this->pi_linkToPage($str,$id,$this->settings['result_link_target'],$urlParameters);
		}
	}
	 ...
	 * @todo should become a viewhelper eventually
	 */
	protected function tellUsWhatIsSeachedFor($searchWords) {
		$searchingFor = '';

			// Traverse search words:
		foreach ($searchWords as $k => $v) {
			if ($k > 0) {
				$operand = strtolower(str_replace(' ', '', $v['oper']));
				$searchingFor .= ' ' . Tx_Extbase_Utility_Localization::translate('searchFor.' . $operand, 'indexed_search');
			} else {
				$searchingFor = Tx_Extbase_Utility_Localization::translate('searchFor', 'indexed_search');
			}
			$searchingFor .= ' ' . $this->utf8_to_currentCharset($v['sword']);
		}
		return $searchingFor;
	}
	 ...
	 */
	protected function utf8_to_currentCharset($str) {
	...
	/***********************************
	 *
	 * Please recycle and remove these functions
	 *
	 ***********************************/
